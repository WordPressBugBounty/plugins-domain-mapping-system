...
export default function WcSubdomainAuthenticationRow( { slug, slugMaps, value, selectValue, updateValue, restUrl, restNonce, isPremium, upgradeUrl, siteUrl, loading } ) {
    const isRtl = isRTL();
        ...
        <div className="dms-n-additional-accordion-li">
            ...
            <div className="dms-n-additional-accordion-content">
                <span className="label">
                    <strong>{__("WooCommerce Subdomain Authentication", 'domain-mapping-system')}</strong> - {__("\n" +
                    "Allow users to log in to WooCommerce via subdomains of your primary domain.", 'domain-mapping-system')}
                </span>
                {showSelect && mappings.length >= 1 && <>
                    &nbsp;<span
                    className="label">{__("Choose domains to activate login:", 'domain-mapping-system')}</span>
                ...
                <span className="label"
                      dangerouslySetInnerHTML={{__html: "&nbsp;" + sprintf(__("Read more in our %sdocumentation%s.", 'domain-mapping-system'), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com/features/subdomain-authentication">', '</a>')}}></span>
                {!isPremium && <>
                    &nbsp;
